...
<div class="space-y-6 max-w-xl">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h2 class="text-2xl font-bold tracking-tight text-slate-900">Registrar Cliente</h2>
            <p class="text-sm text-slate-500">Agrega un nuevo cliente para el negocio.</p>
        </div>
    </div>

    <div class="bg-white rounded-xl shadow-sm border border-slate-200/80 p-6 sm:p-8">
        <form action="{{ route('tenant.admin.customers.store') }}" method="POST" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <input type="text" id="phone" name="phone" required value="{{ old('phone') }}" placeholder="Ej. 3001234567"
                    class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-tenant-primary text-sm text-slate-800 bg-slate-50 focus:bg-white transition-all duration-200">
                @error('phone')
                    <span class="text-xs text-red-500 font-semibold mt-1 block">{{ $message }}</span>
                @enderror
            ...
            <div>
                ...
                <input type="text" id="address" name="address" value="{{ old('address') }}" placeholder="Calle 123 #45-67, Apto 201"
                    class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-tenant-primary text-sm text-slate-800 bg-slate-50 focus:bg-white transition-all duration-200">
                @error('address')
                    <span class="text-xs text-red-500 font-semibold mt-1 block">{{ $message }}</span>
                @enderror
            ...
            <div>
                ...
                <input type="text" id="city" name="city" value="{{ old('city') }}" placeholder="Medellín"
                    class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-tenant-primary text-sm text-slate-800 bg-slate-50 focus:bg-white transition-all duration-200">
                @error('city')
                    <span class="text-xs text-red-500 font-semibold mt-1 block">{{ $message }}</span>
                @enderror
